/**
 * DESHAL ERP — Recurring Schedule & Date Calculation Engine
 * Domain Layer: Recurrence frequency math, due date preview generation, overdue status math,
 * and voucher generation from recurring schedule domain models.
 */
...
export function calculateNextDueDate(
  ...
  const year = parts[0] || fallbackYear;
 ...
 */
export function getTodayYMD(nowMs: number = Date.now()): string {
  return formatDateToYMD(new Date(nowMs));
}
...
export function buildVoucherFromSchedule(
  ...
  const voucherId = `voucher-rec-${nowMs}-${Math.floor(randomEntropy * 1000)}`;
