          ...
          {groupedNavItems.map((group) => (
            <div key={group.id} className="space-y-1">
              ...
              {group.items.map((item) => {
                ...
                const moduleCode = TAB_MODULE_MAP[item.id];
                const isModuleDisabled = moduleCode ? !tenantContext.actions.hasModuleAccess(moduleCode) : false;
                  ...
                  >
                    <Icon
                      className={`w-5 h-5 shrink-0 transition-colors ${
                        isActive ? 'text-indigo-600' : isModuleDisabled ? 'text-slate-300' : 'text-slate-400 group-hover:text-indigo-600'
                      }`}
                    />

                    {!effectiveCollapsed && (
                      <span className={`text-xs sm:text-sm truncate flex-1 ${isModuleDisabled ? 'line-through text-slate-400' : ''}`}>
                        {isRTL ? item.labelAr : item.labelEn}
                      </span>
                    )}

                    {!effectiveCollapsed && isModuleDisabled && (
                      <span className="px-1.5 py-0.5 rounded bg-amber-100 text-amber-800 text-[10px] font-bold flex items-center gap-0.5 shrink-0" title="الموديول غير مفعّل في اشتراك المستأجر">
                        <Lock className="w-3 h-3" />
                      </span>
                    )}

                    {!effectiveCollapsed && !isModuleDisabled && item.badge !== undefined && (
                      <span
